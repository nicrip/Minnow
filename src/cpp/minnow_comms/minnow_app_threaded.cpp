...
void Subscriber::Run() {
  ...
  zmq_connect((void*)subscriber, address_.c_str());
  zmq_setsockopt((void*)subscriber, ZMQ_SUBSCRIBE, topic_.c_str(), topic_.size());
...
void App::Publish(std::string topic, uint8_t* msg, size_t msg_size) {
  ...
  int rc = zmq_send((void*)*zmq_socket_, buffer, (size_t)total_len, 0);
}
...
void App::Run() {
  ...
  zmq_connect((void*)*zmq_socket_, socket.c_str());
  ss.str("");
  if(protocol_ == "ipc") {
    ss << "ipc://" << host_ << ".pub";
  } else if(protocol_ == "tcp") {
